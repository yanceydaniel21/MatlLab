...
y=@(x_1, M_1)   [M_1*(1+0.5*(gamma1-1)*M_1^2)/(1-M_1^2)]*(([(gamma1*(M_1^2)*(f1/D1))])+((1+gamma1*M_1^2)/(2*(T_o1+((Q1/(pi()*D1*L_crit1))*x_1)/(L_crit1*m_dot1*cp1)))*((Q1/(pi()*D1*L_crit1)))/(L_crit1*m_dot1*cp1)));
...
y=@(x_2, M_2)   [M_2*(1+0.5*(gamma2-1)*M_2^2)/(1-M_2^2)]*(([(gamma2*(M_2^2)*(f2/D2))])+((1+gamma2*M_2^2)/(2*(T_o2+((Q2/(pi()*D2*L_crit2))*x_2)/(L_crit2*m_dot2*cp2)))*((Q2/(pi()*D2*L_crit2)))/(L_crit2*m_dot2*cp2)));
...
M3 = 1.8; %M1
gamma3 = 1.4; %for air 
f3 = 0.002; %friction factor

D3 = 6/39.37; %inches to meters
Q3 = 2088*1055.06; %Btu/s to Watts

L_crit3 = 5.75; %used from Fanno Flow
...
y=@(x_3, M_3)   [M_3*(1+0.5*(gamma3-1)*M_3^2)/(1-M_3^2)]*(([(gamma3*(M_3^2)*(f3/D3))])+((1+gamma3*M_3^2)/(2*(T_o3+((Q3/(pi()*D3*L_crit3))*x_3)/(L_crit3*m_dot3*cp3)))*((Q3/(pi()*D3*L_crit3)))/(L_crit3*m_dot3*cp3)));
